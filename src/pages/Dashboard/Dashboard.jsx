...
//     spotifyApi.getTrack(defaultTrackId).then(res => {
...
export default function Dashboard({ code }) {
  ...
  //     <div key={playlist.id} onClick={() => setPlaylistId(playlistId.id) }>

  //       console.log(playlist)
  //       <h3>{playlist.name}</h3>
  //       {/* other details of playlist */}
  //     </div>
  //   ));
  // };
  ...
  const renderUserPlaylists = () => {
    return playlists.map((playlist) => (
      <div key={playlist.id} onClick={() => setSelectedPlaylistId(playlist.id)}>
        <h3>{playlist.name}</h3>
      </div>
    ));
  };
    ...
    <div className="dashStyle">
      <div className="dashStyle__outer-wrap">
        ...
        <div className="dashStyle__map">
          ...
          {renderUserPlaylists()} {/* Corrected usage */}
        </div>
      </div>
      <div className="player">
        <Player accessToken={accessToken} trackUri={playingTrack?.uri} />
